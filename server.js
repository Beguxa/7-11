...
server.get("/getList/searchContent/:s/whatToSearch/:o",function(req,res){
   ...
   if(req.params.o == "book")
   {
        for (x in jsObj)
        {
            if(jsObj[x].title == searchContent || jsObj[x].author == searchContent || jsObj[x].isbn == searchContent)
            {
...
server.get("/delete/id/:id/whatToDelete/:whatToDelete",function(req,resp){
    ...
    for(var i=0; i<len; i++)
    {
        if(jsObj[i] != null && jsObj[i].authorId==req.params.id)
        {
           ...
           var config = fs.readFileSync("./bookData.json");
                        var bookObj = JSON.parse(config);
                        var len = bookObj.length;
                        var j;
                        for(var j=0; j<len; j++)
                        {
                            console.log("Looping..."+bookObj[j].authorId);
                            if(bookObj[j].authorId == req.params.id)
                            {
                                console.log("The Book which Author is to be deleted is found");
                                bookObj[j].author = "";
                                 var newConfigFile = JSON.stringify(bookObj);
                        fs.writeFileSync("./bookData.json",newConfigFile);
                        console.log("The Books with changed Author Name are stored.");
     ...
     var newConfigFile = JSON.stringify(jsObj);
    fs.writeFileSync(filename,newConfigFile);
    console.log("The Contents are updated");
    resp.end("/");
});
